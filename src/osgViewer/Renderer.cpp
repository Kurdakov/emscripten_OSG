...
#else
		_extensions->_glGetQueryObjectiv(query, GL_QUERY_RESULT_AVAILABLE, &available);
#endif
        if (available)
        {
...
#else
			_extensions->_glGetQueryObjectui64v(query, GL_QUERY_RESULT, &timeElapsed);
#endif
...
#else
		_extensions->_glGenQueries(1, &query);
#endif
        return query;
    }
    else
...
#else
	_extensions->_glBeginQuery(GL_TIME_ELAPSED, query);
#endif
...
#else
	_extensions->_glEndQuery(GL_TIME_ELAPSED);
#endif
}
...
#else
		_extensions->_glGenQueries(1, &query.first);
        _extensions->_glGenQueries(1, &query.second);
#endif
    }
    else
    {
...
#else
	_extensions->_glQueryCounter(query.first, GL_TIMESTAMP);
#endif
...
void ARBQuerySupport::endQuery(osg::State* /*state*/)
{
    ...
    _extensions->glQueryCounter(_queryFrameList.back().queries.second,
		                          GL_TIMESTAMP);
#else
	_extensions->_glQueryCounter(_queryFrameList.back().queries.second,
		                          GL_TIMESTAMP);
#endif
        ...
        )
    {
...
#else
		_extensions->_glGetQueryObjectiv(itr->queries.second,
                                        GL_QUERY_RESULT_AVAILABLE, &available);
        ...
        if (available)
        {
            ...
            _extensions->glGetQueryObjectui64v(queries.second, GL_QUERY_RESULT,
                                               &endTimestamp);
#else
			_extensions->_glGetQueryObjectui64v(queries.first, GL_QUERY_RESULT,
                                               &beginTimestamp);
            _extensions->_glGetQueryObjectui64v(queries.second, GL_QUERY_RESULT,
                                               &endTimestamp);

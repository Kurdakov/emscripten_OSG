...
#else
			ext->_glGetQueryObjectiv( tr->_id, GL_QUERY_RESULT, &(tr->_numPixels) );
#endif
            if (tr->_numPixels < 0)
                OSG_WARN << "osgOQ: RQCB: " <<
...
#else
	ext->_glBeginQuery( GL_SAMPLES_PASSED_ARB, tr->_id );
#endif
    osg::Geometry::drawImplementation( renderInfo );
#ifndef EMSCRIPTEN
    ext->glEndQuery( GL_SAMPLES_PASSED_ARB );
#else
	ext->_glEndQuery( GL_SAMPLES_PASSED_ARB );
#endif
    tr->_active = true;
...
#else
			extensions->_glDeleteQueries( 1L, &(*titr ) );
#endif
            titr = qol.erase(titr);
            elapsedTime = timer.delta_s(start_tick,timer.tick());
        }

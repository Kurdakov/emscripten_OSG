...
#ifdef _WIN_EGL
EGLContext  GraphicsWindowWin32::createContextImplementation()
{
    ...
    EGLNativeWindowType eglWindow = (EGLNativeWindowType) _hwnd;

	eglDisplay = eglGetDisplay(_hdc);//EGL_DEFAULT_DISPLAY
      

        EGLint major;
        EGLint minor;
        EGLBoolean eglInitOk = eglInitialize(eglDisplay, &major, &minor);

		if(!eglInitOk)
		{
			reportErrorForScreen( "GL ES2: Failed to initialize EGL.",
        ...
        if (eglChooseConfig(eglDisplay, attributeList, &bestConfig, 1, &numConfigs) == EGL_FALSE)
        {
			reportErrorForScreen( "GL ES2: Failed to choose config.",
                _traits->screenNum, ::GetLastError() );
               
        }

        eglSurface = eglCreateWindowSurface(eglDisplay, bestConfig, eglWindow, NULL);
        if (eglSurface == NULL)
        {
			 reportErrorForScreen( "GL ES2: eglCreateWindowSurface failed.",
                _traits->screenNum, ::GetLastError() );
               
        }
       
        // Create a rendering context
        EGLContext eglContext = eglCreateContext(eglDisplay, bestConfig, EGL_NO_CONTEXT, NULL);
        if (eglContext == NULL)
        {
                reportErrorForScreen( "GL ES2: eglCreateContext failed.",
                _traits->screenNum, ::GetLastError() );
        }

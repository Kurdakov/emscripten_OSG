...
#else
		_extensions->_glGenBuffers(1, &_id);
#endif //EMSCRIPTEN
#ifndef EMSCRIPTEN
    _extensions->glBindBuffer(target, _id);
#else
	_extensions->_glBindBuffer(target, _id);
#endif //EMSCRIPTEN
...
#else
	_extensions->_glBindBuffer(target, 0);
#endif //EMSCRIPTEN
}
...
#else
	_extensions->_glTexBuffer(GL_TEXTURE_BUFFER_ARB, internalFormat, _id);
#endif //EMSCRIPTEN
}
...
#else
	_extensions->_glBufferData(GL_TEXTURE_BUFFER_ARB, image->getTotalDataSize(), image->data(), _usageHint);
#endif //EMSCRIPTEN
}
...
#else
	_extensions->_glBufferSubData(GL_TEXTURE_BUFFER_ARB, 0, image->getTotalDataSize(), image->data());
#endif //EMSCRIPTEN
}

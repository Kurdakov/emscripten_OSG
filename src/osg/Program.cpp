...
void Program::flushDeletedGlPrograms(unsigned int contextID,double /*currentTime*/, double& availableTime)
{
...

    {
        ...
        {
            extensions->_glDeleteProgram( *titr );
...
#else
		_glProgramHandle = _extensions->_glCreateProgram();
#endif
...
void Program::PerContextProgram::linkProgram(osg::State& state)
{
...
#else
		_extensions->_glProgramBinary( _glProgramHandle, programBinary->getFormat(),
			reinterpret_cast<const GLvoid*>(programBinary->getData()), programBinary->getSize() );
		_extensions->_glGetProgramiv( _glProgramHandle, GL_LINK_STATUS, &linked );
#endif
...
#else
			_extensions->_glProgramParameteri( _glProgramHandle, GL_GEOMETRY_VERTICES_OUT_EXT, _program->_geometryVerticesOut );
            _extensions->_glProgramParameteri( _glProgramHandle, GL_GEOMETRY_INPUT_TYPE_EXT, _program->_geometryInputType );
            _extensions->_glProgramParameteri( _glProgramHandle, GL_GEOMETRY_OUTPUT_TYPE_EXT, _program->_geometryOutputType );
#endif
        }
...
#else
			_extensions->_glBindAttribLocation( _glProgramHandle, itr->second, reinterpret_cast<const GLchar*>(itr->first.c_str()) );
#endif
        }
...
#else
			_extensions->_glBindFragDataLocation( _glProgramHandle, itr->second, reinterpret_cast<const GLchar*>(itr->first.c_str()) );
#endif
        }
...
#else
			_extensions->_glProgramParameteri( _glProgramHandle, GL_PROGRAM_BINARY_RETRIEVABLE_HINT, GL_TRUE );
#endif
        }
...
#else
		_extensions->_glLinkProgram( _glProgramHandle );
        //_extensions->_glGetProgramiv( _glProgramHandle, GL_LINK_STATUS, &linked );
		_isLinked =  GL_TRUE;
#endif
    ...
    if (_extensions->isUniformBufferObjectSupported())
    {
        ...
        _extensions->glGetProgramiv(_glProgramHandle,
                                    ...
                                    &maxBlockNameLen);
#else
		_extensions->_glGetProgramiv(_glProgramHandle, GL_ACTIVE_UNIFORM_BLOCKS,
                                    reinterpret_cast<GLint*>(&activeUniformBlocks));
        _extensions->_glGetProgramiv(_glProgramHandle,
                                    GL_ACTIVE_UNIFORM_MAX_LENGTH,
                                    &maxBlockNameLen);
#endif
        ...
        if (maxBlockNameLen > 0)
        {
            ...
            for (GLuint i = 0; i < activeUniformBlocks; ++i)
            {
                ...
                _extensions->glGetActiveUniformBlockiv(_glProgramHandle, i,
                                                       ...
                                                       &blockSize);
#else
				_extensions->_glGetActiveUniformBlockName(_glProgramHandle, i,
                                                         maxBlockNameLen, &len,
                                                         &blockName[0]);
                _extensions->_glGetActiveUniformBlockiv(_glProgramHandle, i,
                                                       GL_UNIFORM_BLOCK_DATA_SIZE,
                                                       &blockSize);
#endif
            ...
            ++itr)
        {
            ...
            if (bitr != bindingList.end())
            {
                ...
                _extensions->glUniformBlockBinding(_glProgramHandle, itr->second._index,
                                                   bitr->second);
#else
				_extensions->_glUniformBlockBinding(_glProgramHandle, itr->second._index,
                                                   bitr->second);
#endif
...
#else
	_extensions->_glGetProgramiv( _glProgramHandle, GL_ACTIVE_UNIFORMS, &numUniforms );
    //_extensions->_glGetProgramiv( _glProgramHandle, GL_ACTIVE_UNIFORM_MAX_LENGTH, &maxLen );
	maxLen = 30;
#endif
...
#else
			_extensions->_glGetActiveUniform( _glProgramHandle,
                    i, maxLen, 0, &size, &type, name );
#endif
...
#else
			GLint loc = _extensions->_glGetUniformLocation( _glProgramHandle, name );
#endif
    ...
    if (_extensions->isShaderAtomicCounterSupported() && !atomicCounterMap.empty())
    {
		...
		_extensions->glGetActiveUniformsiv( _glProgramHandle, uniformIndex.size(),
                                            ...
                                            &(bufferIndex[0]) );
#else
		_extensions->_glGetActiveUniformsiv( _glProgramHandle, uniformIndex.size(),
                                            &(uniformIndex[0]), GL_UNIFORM_ATOMIC_COUNTER_BUFFER_INDEX,
                                            &(bufferIndex[0]) );
#endif
...
#else
		  _extensions->_glGetProgramiv(_glProgramHandle, GL_ACTIVE_ATOMIC_COUNTER_BUFFERS,
                                    reinterpret_cast<GLint*>(&activeAtomicCounterBuffers));
#endif
...
#else
				GLint bindID = 0;
				_extensions->_glGetActiveAtomicCounterBufferiv(_glProgramHandle, i,
                                                              GL_ATOMIC_COUNTER_BUFFER_BINDING,
                                                              &bindID);
...
#else
	_extensions->_glGetProgramiv( _glProgramHandle, GL_ACTIVE_ATTRIBUTES, &numAttrib );
    //_extensions->_glGetProgramiv( _glProgramHandle, GL_ACTIVE_ATTRIBUTE_MAX_LENGTH, &maxLen );
	maxLen = 30;
#endif
...
#else
			_extensions->_glGetActiveAttrib( _glProgramHandle,
                    i, maxLen, 0, &size, &type, name );

            GLint loc = _extensions->_glGetAttribLocation( _glProgramHandle, name );
#endif
...
#else
	_extensions->_glValidateProgram( _glProgramHandle );
    _extensions->_glGetProgramiv( _glProgramHandle, GL_VALIDATE_STATUS, &validated );
#endif
...
#else
	return _extensions->_getProgramInfoLog( _glProgramHandle, infoLog );
#endif
}
...
#else
	_extensions->_glGetProgramiv( _glProgramHandle, GL_PROGRAM_BINARY_LENGTH, &binaryLength );
#endif
    if (binaryLength)
    {
...
#else
		_extensions->_glGetProgramBinary( _glProgramHandle, binaryLength, 0, &binaryFormat, reinterpret_cast<GLvoid*>(programBinary->getData()) );
#endif
...
#else
		_extensions->_glUseProgram( _glProgramHandle  );
#endif
...
#else
		_extensions->_glDispatchCompute( _program->_numGroupsX, _program->_numGroupsY, _program->_numGroupsZ );
#endif

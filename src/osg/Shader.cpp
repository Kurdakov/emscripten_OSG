...
#else
			extensions->_glDeleteShader( *titr );
#endif
...
#else
	_glShaderHandle = _extensions->_glCreateShader( shader->getType() );
#endif
    requestCompile();
}
...
#else
	_extensions->_glShaderSource( _glShaderHandle, 1, &sourceText, NULL );
    _extensions->_glCompileShader( _glShaderHandle );
    _extensions->_glGetShaderiv( _glShaderHandle, GL_COMPILE_STATUS, &compiled );
#endif
...
{
    return _extensions->_getShaderInfoLog( _glShaderHandle, infoLog );
}
...
#else
	_extensions->_glAttachShader( program, _glShaderHandle );
#endif
}
...
#else
	_extensions->_glDetachShader( program, _glShaderHandle );
#endif
}

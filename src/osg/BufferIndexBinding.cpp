...
#else
		glObject->_extensions->_glBindBufferRange(_target, _index,
                                                 glObject->getGLObjectID(), _offset, _size);
#endif
...
#else
		bo->_extensions->_glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, bo->getGLObjectID());
#endif
         ...
         GLubyte* src = (GLubyte*)bo->_extensions->glMapBuffer(GL_ATOMIC_COUNTER_BUFFER,
                                                          GL_READ_ONLY_ARB);
#else
		 GLubyte* src = (GLubyte*)bo->_extensions->_glMapBuffer(GL_ATOMIC_COUNTER_BUFFER,
                                                          GL_READ_ONLY_ARB);
#endif
...
#else
		bo->_extensions->_glUnmapBuffer(GL_ATOMIC_COUNTER_BUFFER);
#endif
...
#else
		bo->_extensions->_glBindBuffer(GL_ATOMIC_COUNTER_BUFFER, static_cast<GLuint>(previousID));
#endif
